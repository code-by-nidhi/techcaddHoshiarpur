        ...
        <ul className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {courses.map((c) => (
            <motion.li key={c.slug} variants={fadeUp} className="h-full">
              ...
              >
                {/* 4:3, matching every banner in the catalogue: a 16/10 band cropped
                    17% off the top and bottom of each one. */}
                <div className="relative aspect-[4/3] w-full overflow-hidden bg-[#0A1437]">
                  <Image
                    src={c.heroImage}
                    alt={c.title}
                    fill
