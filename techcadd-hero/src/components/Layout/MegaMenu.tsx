  ...
  return (
    <motion.div variants={panelIn} initial="hidden" animate="show" exit="exit" className="relative">
      ...
      <span
        aria-hidden
        style={{ left: arrow }}
        className="absolute -top-[7px] size-3.5 -translate-x-1/2 rotate-45 rounded-[3px] border-l border-t border-black/[0.08] bg-white"
      />

      <div className="relative overflow-hidden rounded-[16px] border border-black/[0.08] bg-white shadow-[0_12px_32px_rgba(0,0,0,0.12)] backdrop-blur-[10px]">
        ...
        <span
          aria-hidden
          className="pointer-events-none absolute inset-x-0 top-0 h-28 bg-[linear-gradient(180deg,rgba(37,99,235,0.04),transparent)]"
        />
        {MOTES.map((m, i) => (
          <motion.span
            key={i}
        ...
        <div className="row g-0">
          ...
          <div className="col-12 col-lg-3">
            <motion.ul
              variants={itemIn}
              className="relative h-full border-b border-black/[0.06] bg-[linear-gradient(180deg,rgba(37,99,235,0.05),transparent)] p-4 lg:border-b-0 lg:border-r"
            >
              {RESOURCES.map((item) => {
                    ...
                    <Link
                      ...
                      className={`group/row mb-1 flex items-center gap-3 rounded-2xl px-3.5 py-3 text-[14px] transition-[background-color,color,transform] duration-300 ${
                        on
                          ...
                          : "text-[#111827] hover:bg-[#F3F6FF] hover:text-[#2563EB]"
                      }`}
                    >
                      <span
                        className={`grid size-8 shrink-0 place-content-center rounded-xl transition-colors duration-300 ${
                          on ? "bg-white/20 text-white" : "bg-[#2563EB]/10 text-[#2563EB]"
                        }`}
                      >
  ...
  return (
    <motion.div variants={itemIn} className="h-full">
      ...
      >
        ...
        <div className="relative flex h-full flex-col overflow-hidden rounded-[23px] border border-black/[0.08] bg-white shadow-[0_14px_36px_-26px_rgba(15,23,42,0.45)] backdrop-blur-[10px] transition-shadow duration-500 group-hover/card:shadow-[0_30px_64px_-30px_rgba(37,99,235,0.55)]">
          ...
          <div className="flex flex-1 flex-col p-4">
            <div className="flex items-center gap-2">
              <h3 className="font-[family-name:var(--font-sora)] text-[15.5px] font-bold tracking-[-0.015em] text-[#111827]">
                {card.title}
              </h3>
